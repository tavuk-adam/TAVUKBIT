...
# Modern Widescreen UI (16:9), Dark/Light Mode ve Ayarlanabilir Olasılık (Max 25) ile Güncellendi
...
fiyat = 10  # Mevcut TAVUKBIT fiyatı (Tam sayı)
log_kaydi = []  # Olay günlüğü
simulasyon_aktif = False  # Simülasyonun aktiflik bayrağı
kalan_sure = 0  # Kalan süre
dusme_meille_seviye = 0
yukselme_meille_seviye = 0
MAX_MEILLE_LEVEL = 25  # Maksimum olasılık ayar seviyesi

# KULLANICI HESAPLARI (In-memory storage)
# Format: {username: {'password': '...', 'elmas': 1000, 'tavukbit': 0, 'is_admin': False}}
users = {
...
# HTML şablonu (Modern Widescreen UI)
HTML = '''
<!doctype html>
...
<body class="container-fluid py-5">
                    ...
                    {% if not session.get("giris_tavuk") %}
                        <div class="card p-5 shadow-lg border-0">
                            ...
                            <p class="text-center text-muted mt-3 mb-0 small">
                                (TESTUSER ve ADMIN hesapları otomatik tanımlıdır)
                            </p>
                        </div>
                    {% else %}
                        ...
                        <div class="card p-4 mb-4">
                            ...
                            <div class="row align-items-center bg-light-subtle rounded p-3">
                                <div class="col-4 border-end border-secondary">
                                    ...
                                    <div class="fiyat text-success" id="fiyat">{{ fiyat }}</div>
    ...
    <script>
        ...
        function setupAdminForm(formId, url, successMessageId) {
            ...
            form.onsubmit = (e) => {
                e.preventDefault();
                const inputs = Array.from(form.elements).filter(el => el.name !== 'submit' && el.value.trim() !== '');
                const data = {};
                inputs.forEach(input => {
                    data[input.id.replace(formId === 'register-form' ? 'reg_' : 'upd_', '')] = input.type === 'number' ? parseInt(input.value) : input.value;
                });
                ...
                .then(result => {
                    if (result.success) {
                        displayMessage(successMessageId, result.message, false);
                        form.reset();
                    } else {
                        displayMessage(successMessageId, result.message, true);
                    }
                    update();
                })
                .catch(error => {
...
def simulasyonu_baslat(sure, baslangic=None):
    ...
    for saniye in range(1, sure + 1):
        ...
        with lock:
            ...
            # Düşüş/Yükseliş olasılıklarını artırmak için ilgili ağırlıkları (ilk iki ve son iki) artır
...
def register_user():
    ...
    initial_elmas = data.get("elmas", 10000)  # HTML'de reg_elmas'ın id'sini elmas olarak gönderiyorum.
...
def durdur():
    ...
    with lock: simulasyon_aktif = False
    return ('', 204)
...
def meille_dusme_artir():
    ...
    global dusme_meille_seviye, yukselme_meille_seviye
    with lock:
        if dusme_meille_seviye < MAX_MEILLE_LEVEL:  # MAX 25 KONTROLÜ
...
def meille_yukselme_artir():
    ...
    with lock:
        if yukselme_meille_seviye < MAX_MEILLE_LEVEL:  # MAX 25 KONTROLÜ
